...
import { listActiveSellers, listPublicCategories, listPublicProjects, publicCounts } from "@/db/queries/public";

export const dynamic = "force-dynamic";

export default async function HomePage() {
  const [sellers, categories, projects, counts] = await Promise.all([listActiveSellers(), listPublicCategories(), listPublicProjects(6), publicCounts()]);
  ...
  return <main id="main" className="app-shell">
    ...
    <section className="hero"><span className="eyebrow">تصميم يُفصّل على حياتك</span><h1>مساحات هادئة، وخزائن مصنوعة لتبقى.</h1><p>استكشف مجموعة منتقاة من أعمال الحناوي، وتواصل مباشرة مع مستشار التصميم الذي يناسب مشروعك.</p><div style={{ display: "flex", justifyContent: "center", gap: ".6rem", flexWrap: "wrap" }}><Link className="button button-primary" href={sellers[0] ? `/s/${sellers[0].slug}` : "/works"}>ابدأ الاستكشاف <Sparkles size={18} /></Link><Link className="button" href="/works">مكتبة المشاريع <Layers3 size={18} /></Link></div></section>
