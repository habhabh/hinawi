...
export async function GET(request: Request, { params }: { params: Promise<{ opaqueToken: string }> }) {
  ...
  if (!qrTokenSchema.safeParse(opaqueToken).success) return NextResponse.redirect(new URL("/?qr=invalid", request.url));
  const rows = await db.select({ qr: qrLinks, slug: sellers.slug, sellerId: sellers.id }).from(qrLinks).innerJoin(sellers, eq(sellers.id, qrLinks.sellerId)).where(and(eq(qrLinks.token, opaqueToken), eq(qrLinks.isActive, true), eq(sellers.isActive, true))).limit(1);
  const item = rows[0];
  ...
  const response = NextResponse.redirect(new URL(`/s/${item.slug}`, request.url), 307);
